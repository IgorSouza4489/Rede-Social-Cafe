...
namespace CafeJWTMVC.Controllers
{
    ...
    {
        public static string baseUrl = "http://localhost:62049/api/regiaos/";
        ...
        [HttpGet]
        public async Task<List<Regiao>> GetProducts()
        {
            ...
            var res = JsonConvert.DeserializeObject<List<Regiao>>(jsonStr).ToList();
        ...
        public async Task<IActionResult> Create([Bind("")] Regiao products)
        {
            var accessToken = HttpContext.Session.GetString("JWToken");
            var url = baseUrl;
            HttpClient client = new HttpClient();
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            string jsonStr = await client.GetStringAsync(url);
            var res = JsonConvert.DeserializeObject<Regiao>(jsonStr);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("")] Regiao products)
        {
            if (id != products.RegiaoId)
            {
                return NotFound();
            }
            var accessToken = HttpContext.Session.GetString("JWToken");
